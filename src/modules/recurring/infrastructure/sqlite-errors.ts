/**
 * Translation of SQLite failures into recurrence refusals.
 *
 * The unique index on rule and scheduled day is the authority on whether a due
 * date was already processed, so the adapter attempts the reservation and reads
 * the verdict from the driver instead of looking first and hoping no other
 * writer gets in between. That is what makes two concurrent runs resolve into
